...
def main():
    ...
    if not iswebhookFile:
        ...
        webhooklayout = [
            ...
            [
                sg.Text("Message:"),
                sg.Push(),
                sg.Multiline(key="-WHMESSAGE-", size=(50, 6)),
            ],
            [
                sg.FileBrowse("Browse", key="-FILEBROWSE-"),
                sg.Push(),
                sg.InputText(filepath, key="-FILEPATHWH-", do_not_clear=False),
            ],
            [
                sg.Button("Send", key="-WEBHOOKSEND-"),
                sg.Button("Exit", key="-EXIT0-"),
                sg.Push(),
                sg.Text("Max Filesize: 1024Kb"),
                sg.Push(),
            ],
        ]
            ...
            if not fileread:
                ...
                webhooklayout = [
                    [
                        sg.Text("Webhook_URL:"),
                        sg.Push(),
                        sg.Input(webhook_split, key="-WEBHOOK-"),
                    ],
                    [
                        sg.Text("Webhook_Key:"),
                        sg.Push(),
                        sg.Input(webhook_key, key="-KEY-"),
                    ],
                    [
                        sg.Text("Message:"),
                        sg.Push(),
    ...
    if not ismqttFile:
        ...
        mqttlayout = [
            [
                sg.Text("Server URL:"),
                sg.Push(),
                sg.Input("http://mqtt.grouplotse.com", key="-MQTTBROKER-"),
            ],
            ...
            [
                sg.Text("Message:"),
                sg.Push(),
                sg.Multiline(key="-MQTTMESSAGE-", size=(50, 6)),
            ],
            ...
            if not fileread:
                ...
                mqttlayout = [
                    ...
                    [
                        sg.Text("Username:"),
                        sg.Push(),
                        sg.Input("", key="-MQTTUSERNAME-"),
                    ],
                    [
                        sg.Text("Password:"),
                        sg.Push(),
                        sg.Input("", key="-MQTTPASSWORD-"),
                    ],
                    [
                        sg.Text("Message:"),
                        sg.Push(),
                ...
                mqttlayout = [
                    [
                        sg.Text("Server URL:"),
                        sg.Push(),
                        sg.Input("mqtt.grouplotse.com", key="-MQTTBROKER-"),
                    ],
                    [sg.Text("Port:"), sg.Push(), sg.Input("1883", key="-MQTTPORT-")],
                    ...
                    [
                        sg.Text("Username:"),
                        sg.Push(),
                        sg.Input(username, key="-MQTTUSERNAME-"),
                    ],
                    [
                        sg.Text("Password:"),
                        sg.Push(),
                        sg.Input(password, key="-MQTTPASSWORD-"),
                    ],
                    [
                        sg.Text("Message:"),
                        sg.Push(),
    ...
    aboutlayout = [
        [
            sg.Multiline(
                "GroupLotse is a virtual assistant that enables quick clarification of responsibilities\n"
            ...
            sg.Button("", image_filename="gl_icon.png", key="-WEBSITEBUTTON-"),
        ]
    ]
    ...
    tab_group_layout = [
        [
            sg.Tab("Webhook", webhooklayout, key="-TAB0-"),
        ...
        if event == "-WEBHOOKSEND-":
            ...
            if len(filepath) > 3:
                ...
                if file_size <= 1048576 and file_extension in allowed_file_extensions:
                    Grouplotse.webhook_file_post(
                        Grouplotse, webhook, key, filepath, file_extension
                    )
